...
class ChattyBot:
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not update.effective_user or not self._is_allowed_user(update.effective_user.id):
            logger.info(f"Ignoring /start from unauthorized user: {update.effective_user.id if update.effective_user else 'unknown'}")
            return
    ...
    async def facts_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
